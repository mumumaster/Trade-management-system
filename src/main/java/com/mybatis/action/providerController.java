...
import com.mybatis.bean.ProviderExample.Criteria;
...
public class providerController extends BaseController{
	...
	private Logger logger = Logger.getLogger(providerController.class);
		@Resource
		private ProviderService   provice;
		...
		public Msg showResponseBody(Model model, @RequestParam("proname") String proName,
			...
			list = provice.selectByExample(example);

			// 使用PageInfo对象可以得到分页的各项属性，5表示每页只显示5条连续的页数
			PageInfo<Provider> pageInfo = new PageInfo<Provider>(list);
		

			return Msg.success().add("pageInfo", pageInfo).add("selectName", proName);
		}
		...
		public Msg adds(@Valid @ModelAttribute("provider") Provider provider,HttpSession session) {					
		
			int result = provice.insert(provider);		
			 if (result != 0) {
				 return Msg.success();
		        } else {
		          return Msg.fail();
		...
		public Msg getEmp(@Param("id")Integer id){
			Provider proInfo = provice.selectByPrimaryKey(id);
			return Msg.success().add("Provider", proInfo);
		}
		...
		public Msg modify(Provider proinfo,HttpServletRequest request){						
			System.out.println("学生的ID:"+proinfo.getId());
			System.out.println("将要更新的员工数据："+proinfo);
			provice.updateByPrimaryKeySelective(proinfo);
			return Msg.success();
		}
